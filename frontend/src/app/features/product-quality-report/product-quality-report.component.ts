...
@Component({
      ...
      <header class="page-header">
        <div class="header-top">
          ...
          <div class="header-actions">
            <button class="btn-theme" (click)="settingsService.toggleTheme()" [title]="settingsService.isDark() ? 'Switch to light mode' : 'Switch to dark mode'">
              {{ settingsService.isDark() ? '☀️' : '🌙' }}
            </button>
      ...
      @if (data(); as d) {
        ...
        <section class="charts-row">
          ...
          <div class="chart-panel">
            ...
            <p class="chart-desc">Open Critical &amp; Major bugs over the last 6 months</p>
        ...
        <section class="tables-row">
          ...
          <div class="table-panel">
            ...
            @if (d.criticalAndBlockerTickets.length === 0) {
              ...
              <div class="ticket-table-wrap">
                <table class="ticket-table">
                  ...
                  <tbody>
                    @for (t of d.criticalAndBlockerTickets; track t.key) {
          ...
          <div class="table-panel">
            ...
            <p class="generated-at">Generated: {{ d.generatedAt | date:'medium' }}</p>
          </div>
        </section>
  ...
  styles: [`
    ...
    .kpi-value.pct { font-size: 1.8rem; color: var(--accent-blue); }
    ...
    .age-warn { color: #f59e0b; font-weight: 700; }
    ...
    .generated-at { font-size: 0.73rem; color: var(--text-tertiary); margin-top: auto; padding-top: 0.85rem; }
  `]
})
export class ProductQualityReportComponent implements OnInit {
  ...
  monthlyChartData = computed<ChartData<'bar'> | null>(() => {
    ...
    return {
      ...
      datasets: [
        { label: 'Blocker',  data: d.monthlyBugsByMonth.map(m => m.blocker),  backgroundColor: '#dc3545', stack: 'bugs' },
        { label: 'Critical', data: d.monthlyBugsByMonth.map(m => m.critical), backgroundColor: '#fd7e14', stack: 'bugs' },
        { label: 'Major',    data: d.monthlyBugsByMonth.map(m => m.major),    backgroundColor: '#ffc107', stack: 'bugs' },
        { label: 'Minor',    data: d.monthlyBugsByMonth.map(m => m.minor),    backgroundColor: '#28a745', stack: 'bugs' },
      ]
    };
  });
    ...
    return {
      ...
      datasets: [
        { label: 'Critical', data: d.backlogGrowth.map(p => p.critical),
          borderColor: '#fd7e14', backgroundColor: 'rgba(253,126,20,0.25)',
          borderWidth: 1.5, tension: 0.4, pointRadius: 2.5, fill: true },
        { label: 'Major', data: d.backlogGrowth.map(p => p.major),
          borderColor: '#ffc107', backgroundColor: 'rgba(255,193,7,0.20)',
          borderWidth: 1.5, tension: 0.4, pointRadius: 2.5, fill: true },
      ]
    };
  });
  ...
  monthlyChartOptions: ChartConfiguration<'bar'>['options'] = {
    ...
    scales: {
      x: { stacked: true, ticks: { font: { size: 9 }, maxRotation: 45, minRotation: 25 } },
      y: { stacked: true, beginAtZero: true, ticks: { stepSize: 1 } }
    }
  };

  weekToWeekChartOptions: ChartConfiguration<'line'>['options'] = {
    responsive: true, maintainAspectRatio: false,
    plugins: {
  ...
  async onExport(format: SnapshotFormat): Promise<void> {
    ...
    await this.snapshotService.exportSnapshot({
      ...
      chartCanvasIds: ['monthly-bugs-chart', 'week-to-week-chart', 'backlog-growth-chart'],
    });
  }
}
